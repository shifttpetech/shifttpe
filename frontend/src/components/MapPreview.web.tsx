// Web build of MapPreview — does NOT import react-native-maps (native-only).
  ...
  return (
    <Pressable onPress={openInMaps} style={[styles.wrap, { height, backgroundColor: colors.primaryTint }]}>
      <View style={styles.grid}>
        {Array.from({ length: 6 }).map((_, i) => (<View key={i} style={[styles.row, { top: i * 36 }]} />))}
        {Array.from({ length: 6 }).map((_, i) => (<View key={`c-${i}`} style={[styles.col, { left: i * 56 }]} />))}
      </View>
